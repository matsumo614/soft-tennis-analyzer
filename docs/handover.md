...
## 9. 2026-08-18 に修正した不具合

「処理に失敗しました: ... models/gemini-2.5-flash is no longer available to new users」が
頻繁に出ていた問題を修正した。

表示されていた404は**本当の原因ではない**。フォールバックの並びは
`gemini-flash-lite-latest` → `gemini-flash-latest` → `gemini-2.5-flash` で、
最初の2つが429/503で3回ずつ失敗したときだけ3番目に到達する。
その3番目が廃止済みモデルで即404になるため、混雑が原因の失敗が
「モデルが存在しない」という別の顔をして表示されていた。

| 対応 | 内容 |
|---|---|
| フォールバック先の更新 | 既定値を `gemini-flash-latest,gemini-3.6-flash` に変更（`gemini-2.5-flash` は2026-10-20廃止予定で、新しいAPIキーからは既に使えない） |
| 廃止モデルで止まらないように | 404系のエラーは再試行せず次のモデルへ進む（従来はチェーン途中の404で全体が中断していた） |
| エラー表示の改善 | 全モデル失敗時は最後の1件ではなく、試したモデルと理由をまとめて返す |

---

## 10. 注意事項

- Geminiのモデルは入れ替わりが速い。`gemini-2.5-flash` / `gemini-2.5-pro` は2026-10-20に廃止予定で、
  新しいAPIキーからは既に404になる。現行のFlash系は `gemini-3.6-flash`、`gemini-3.5-flash-lite` など。
  `gemini-flash-latest` は2026-08時点で `gemini-3.5-flash` を指す
...
## 11. 2026-09-15 に修正した不具合

本番 https://soft-tennis-analyzer.onrender.com/ の音声文字起こしがうまくいかない問題。

原因はコードではなく、**Renderダッシュボードの環境変数が古いままコードを上書きしていたこと**。
`/api/health` は `model: gemini-flash-lite-latest`、フォールバックに廃止済みの `gemini-2.5-flash` を返していた。
lite は15分音声で繰り返しループする既知の弱点で、8/15にコード側では止めたが、ダッシュボード値が優先されるため本番では直っていなかった。
8/18のフォールバック修正も `main` へ未pushのため本番に未反映だった。

| 対応 | 内容 |
|---|---|
| 環境変数を信用しすぎない | 名前に `lite` が付くモデルと `gemini-2.5-*` / `gemini-2.0-*` は無視する |
| 専用ASRを使う | 文字起こしは `gemini-3.5-transcribe`（日本語ヒント・ソフトテニス用語）→ 失敗時のみ Flash |
| 繰り返し検知 | 同じ文言が何度も出たら次のモデルへ切り替える |
| トークン事前確認 | 失敗しても処理を止めない（本命の文字起こしへ進む） |
